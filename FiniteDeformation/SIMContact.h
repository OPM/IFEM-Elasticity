...
private:
  ...
  public:
    ...
    explicit hasBody(const RigidBody* b) : body(b) {}
...
protected:
  ...
  //! \param[in] model The flexible FE model
  //! \param[in] entitys Set of all named topological entities in the model
  bool parseContactTag(const tinyxml2::XMLElement* elem,
                       const std::vector<ASMbase*>& model,
  ...
  void printBodyReactions(const SAM& sam, const std::vector<double>& RF,
