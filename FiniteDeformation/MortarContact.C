...
void MortarMats::initialize (char)
{
  phiA.init();
  gNA.fill(0.0);
  AA.fill(0.0);
...
MortarContact::MortarContact (RigidBody* mst, GlobalIntegral* gi, usint n)
  : IntegrandBase(n), myInt(gi), master(mst)
{
					 ...
					 const MortarMats& mortar) const
{
  for (size_t n = 0; n < activeSlave.size(); n++)
    if (activeSlave[n])
    {
      std::map<size_t,char>::iterator it = cStat.find(n+1);
      if (it == cStat.end())
	cStat[n+1] = 'A';
      else
	it->second = 'R';
  ...
  for (const std::pair<const size_t,char>& node : cStat)
  {
    std::cout <<"  Contact node "<< node.first;
    switch (node.second)
      {
    ...
    std::cout <<", wgap = "<< mortar.weightedGap(node.first) << std::endl;
  }
}


MortarPenalty::MortarPenalty (RigidBody* mst, const MortarMats& mats, usint n)
  : MortarContact(mst,nullptr,n), mortar(mats)
{
...
MortarAugmentedLag::MortarAugmentedLag (RigidBody* mst, const MortarMats& mats,
					const std::map<int,int>& alMap, usint n)
  : MortarContact(mst,nullptr,n), mortar(mats), ALmap(alMap)
{
