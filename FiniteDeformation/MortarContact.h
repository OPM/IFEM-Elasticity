...
public:
  ...
  //! \param[in] n Number of space dimensions
  MortarContact(RigidBody* mst, GlobalIntegral* gi, usint n);
...
public:
  ...
  //! \param[in] n Number of space dimensions
  MortarPenalty(RigidBody* mst, const MortarMats& mats, usint n);
  //! \brief Empty destructor.
  virtual ~MortarPenalty() {}

  using MortarContact::initIntegration;
  ...
  using MortarContact::getLocalIntegral;
  //! \brief Returns a local integral container for the given element.
  //! \param[in] nen Number of nodes on element
  virtual LocalIntegral* getLocalIntegral(size_t nen, size_t, bool) const;
  //! \brief Returns the system quantity to be integrated by \a *this.
  ...
  using MortarContact::initElementBou;
  //! \brief Initializes current element for boundary integration.
  //! \param[in] MNPC Matrix of nodal point correspondance for current element
  //! \param[in] elmInt Local integral for element
  virtual bool initElementBou(const std::vector<int>& MNPC,
...
class MortarAugmentedLag : public MortarContact
{
  ...
  public:
    ...
    explicit ALElmMats(size_t nedof = 0);
    //! \brief Empty destructor.
    virtual ~ALElmMats() {}
