...
{
  for (RigidBody* body : myBodies) delete body;
  for (ScalarFunc* func : myFuncs) delete func;
}
...
bool SIMContact::addLagrangeMultipliers (ASMbase* patch, int& ngnod)
{
  ...
  size_t iel = patch->getNoElms(true);
...
bool SIMContact::parseContactTag (const tinyxml2::XMLElement* elem,
                                  const std::vector<ASMbase*>& model,
                                  const TopologySet& entitys)
{
  ...
  for (; child; child = child->NextSiblingElement())
  {
    ...
    const tinyxml2::XMLElement* c = child->FirstChildElement();
    for (; c; c = c->NextSiblingElement())
    {
      const tinyxml2::XMLNode* cval = c->FirstChild();
				    ...
				    const SAM& sam, size_t nsd)
{
  ...
  for (RigidBody* body : myBodies)
	...
	return false;
      ...
      itgs.insert(std::make_pair(body->code,new MortarContact(body,mats,nsd)));
      // Integrand for the residual and tangent contributions
      itgs.insert(std::make_pair(body->code,contactInt));
    }
...
{
  for (std::pair<MPC* const,ScalarFunc*>& mpc : dMap)
    if (mpc.first && mpc.second)
      mpc.first->setSlaveCoeff((*mpc.second)(time));
}
...
bool SIMContact::writeGlvBodies (VTF* vtf, int& nBlock)
{
  ...
  for (RigidBody* body : myBodies)
  {
    if (!vtf->writeGrid(body->tesselate(),bodyName.c_str(),++nBlock))
      return false;
...
bool SIMContact::writeGlvBodyMovements (VTF* vtf, int iStep, int& nBlock) const
{
  ...
  for (const RigidBody* body : myBodies)
  {
    if (!vtf->writeTransformation(body->getPosition(),identity,++nBlock,
                                  body->gBlock))
      return false;
...
void SIMContact::printBodyReactions (const SAM& sam,
                                     ...
                                     std::streamsize precision) const
{
  ...
  for (const RigidBody* body : myBodies)
  {
    Vec3 react;
    Vector vec;
    for (size_t j = 1; j <= body->getNoNodes(); j++)
      if (sam.getNodalReactions(body->getNodeID(j),RF,vec))
	react += Vec3(vec);
