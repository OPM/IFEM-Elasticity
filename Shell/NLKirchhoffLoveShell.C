...
void NLKirchhoffLoveShell::setMode (SIM::SolutionMode mode)
{
  ...
  if (mode == SIM::STATIC || mode == SIM::ARCLEN || mode == SIM::RHS_ONLY)
  {
    iS = 1;
    primsol.resize(1);
  }
                                    ...
                                    const Vec3& X) const
{
  ...
  if (eS) // Integrate the load vector due to gravitation and other body forces
  {
    ...
    this->formBodyForce(elMat.b[eS-1],elMat.c,fe.N,fe.iGP,X,n,fe.detJxW);
    if (gS && !presFld.empty())
      this->formBodyForce(elMat.b[gS-1],elMat.c,fe.N,fe.iGP,X,n,fe.detJxW,true);
  }
